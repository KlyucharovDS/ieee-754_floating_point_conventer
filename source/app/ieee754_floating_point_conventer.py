...
class Bin2SingleConversion:
    __MIN = single(-3.40282346639e+38)
    __MAX = single(3.40282346639e+38)
    def __init__(self, *,s_value=None, b_value=None):
        """
        Initializing of begin values
        :param s_value: the entered decimal value as the string type
        :param b_value: the entered binary value as the string type
        """
    ...
    def __check_range_single(self, value:single) -> bool:
        """
        Return a decition of the occurrence of a single number in the range
        :return: True when single value whithin range of acceptable values, otherwise False
        """
        if  (not math.isclose(value,self.__MAX) and value > self.__MAX) or \
            (not math.isclose(value,self.__MIN) and value < self.__MIN):
            return False
        else:
            return True

    def __check_range_binary(self, value:str) -> bool:
        """
        Return a decition of the occurrence of a binary number in the range
        :return: True when single value whithin range of acceptable values, otherwise False
        """
        # 0b011111110111111111111111111111111
        value = value[1::-1]
        if len(value) == 32:
            # for positive & negative numbers
            for i in range(len(value)-1):
                if i !=23 and value[i] == "0":
                  return True
                if value[23] == "1":
                    return False
                else:
...
if "__main__" == __name__:
    ...
    value = Bin2SingleConversion(s_value=single_value)
    bin_value = value.get_bin()
    print(f'{bin_value=}')
    print(f'hex_value={value.get_single()}')
    print('\n+1')
    value.add_min()
